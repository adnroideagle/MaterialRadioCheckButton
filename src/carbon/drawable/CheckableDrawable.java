...
/**
 * Created by Marcin on 2015-03-06.<br/>
 * 【From】https://github.com/ZieIony/material-checkbox-radiobutton
 */
public class CheckableDrawable extends Drawable {
	...
	Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG), maskPaint = new Paint(
			Paint.ANTI_ALIAS_FLAG);
	PorterDuffXfermode porterDuffClear = new PorterDuffXfermode(
			PorterDuff.Mode.CLEAR);
	PorterDuffXfermode porterDuffSrcIn = new PorterDuffXfermode(
	...
	public CheckableDrawable(Context context, int checkedRes, int uncheckedRes,
			int filledRes, PointF offset) {
		...
		try {
			{
				...
				checkedBitmap = Bitmap.createBitmap(bounds.width(),
				...
				checkedShader = new BitmapShader(checkedBitmap,
			...
			}
			{
				...
				uncheckedBitmap = Bitmap.createBitmap(bounds.width(),
						bounds.height(), Bitmap.Config.ARGB_8888);
			...
			}
			{
				...
				filledBitmap = Bitmap.createBitmap(bounds.width(),
						bounds.height(), Bitmap.Config.ARGB_8888);
				...
				filledShader = new BitmapShader(filledBitmap,
						Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
				Matrix matrix = new Matrix();
				matrix.postTranslate(bounds.left, bounds.top);
			...
			maskBitmap = Bitmap.createBitmap(bounds.width(), bounds.height(),
					Bitmap.Config.ARGB_8888);
			maskCanvas = new Canvas(maskBitmap);
			radius = (float) (Math.sqrt(2) * bounds.width() / 2);
		} catch (NullPointerException e) {
		} catch (SVGParseException e1) {
		...
		if (animating) {
			if (checked && checkedBitmap != null) {
				if (delta < CHECK_DURATION) {
					...
					canvas.drawBitmap(uncheckedBitmap, bounds.left, bounds.top,
							paint);

					maskCanvas.drawColor(0xffffffff);
					maskPaint.setXfermode(porterDuffClear);
					maskCanvas.drawCircle(maskBitmap.getWidth() / 2,
							maskBitmap.getHeight() / 2, (1 - delta
									/ FILL_DURATION)
									* radius, maskPaint);
					maskPaint.setXfermode(porterDuffSrcIn);
					maskCanvas.drawBitmap(filledBitmap, 0, 0, maskPaint);
					canvas.drawBitmap(maskBitmap, bounds.left, bounds.top,
							paint);
				} else {
					...
					maskCanvas.drawCircle(
							maskBitmap.getWidth() / 2 + bounds.width()
									* offset.x, maskBitmap.getHeight() / 2
									+ bounds.height() * offset.y,
							(delta - CHECK_DURATION) / FILL_DURATION * radius,
							maskPaint);
					maskPaint.setXfermode(porterDuffSrcIn);
					maskCanvas.drawBitmap(filledBitmap, 0, 0, maskPaint);
					canvas.drawBitmap(maskBitmap, bounds.left, bounds.top,
					...
					canvas.drawCircle(bounds.centerX() + bounds.width()
							* offset.x, bounds.centerY() + bounds.height()
							* offset.y, (delta - CHECK_DURATION)
							/ FILL_DURATION * radius, paint);
				}
				...
				if (delta < CHECK_DURATION) {
					...
					maskCanvas.drawCircle(
							maskBitmap.getWidth() / 2 + bounds.width()
									* offset.x, maskBitmap.getHeight() / 2
									+ bounds.height() * offset.y, (1 - delta
									/ FILL_DURATION)
									* radius, maskPaint);
					maskPaint.setXfermode(porterDuffSrcIn);
					maskCanvas.drawBitmap(filledBitmap, 0, 0, maskPaint);
					...
					canvas.drawCircle(bounds.centerX() + bounds.width()
							* offset.x, bounds.centerY() + bounds.height()
							* offset.y, (1 - delta / FILL_DURATION) * radius,
							paint);
				} else {
					paint.setColorFilter(uncheckedFilter);
					paint.setAlpha(uncheckedAlpha);
	...
	public void setColor(ColorStateList colorStateList) {
		...
		setCheckedColor(color.getColorForState(
				new int[] { android.R.attr.state_checked },
				color.getDefaultColor()));
		setUncheckedColor(color.getColorForState(
				new int[] { -android.R.attr.state_checked },
				color.getDefaultColor()));
		setDisabledColor(color.getColorForState(
